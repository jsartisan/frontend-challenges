...
import type { ChallengeList, Difficulty } from "~/entities/challenge/model/types";

import { insertInfoReadme } from "~/scripts/utils/readme";
import { SUPPORTED_LOCALES } from "~/shared/config/locale";
...
import { getAllChallengesTags, getChallengesByTag, getChallenges } from "~/entities/challenge/api";
import {
  getDifficultyPlainText,
  getDifficultyBadgeInverted,
  getBadge,
  getChallengeBadge,
} from "~/scripts/utils/readme";
...
async function updateIndexREADME(challenges: ChallengeList) {
  ...
  for (const locale of SUPPORTED_LOCALES) {
    ...
    for (const challenge of challengesByDifficulty) {
      if (prev !== challenge.info[locale]?.difficulty)
        challengesREADME += `${prev ? "<br><br>" : ""}${getDifficultyBadgeInverted(
          challenge.info[locale]?.difficulty as Difficulty,
          locale,
    ...
    const tags = getAllChallengesTags(challenges, locale);
 ...
 * @param quizzes
 */
async function updateQuestionsREADME(quizzes: ChallengeList) {
