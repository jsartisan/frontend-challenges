...
export function MockInterviewMachingCodingRoundPage(props: MockInterviewAlgoRoundPageProps) {
  const { round } = props;
...
export function Question(props: QuestionProps) {
  ...
  return (
    <SandpackRoot
      ...
      className="p-6!"
    >
      <QuestionInner challenge={challenge} template={template} setTemplate={setTemplate} round={round} />
    </SandpackRoot>
  );
}
...
function QuestionInner(props: QuestionInnerProps) {
  const { challenge, round, setTemplate, template } = props;
  ...
  return (
    <div className="flex h-full w-full flex-col gap-4">
      <div className="min-h-auto! relative flex w-full justify-between">
        <div>
          <h1 className="h-8 text-2xl font-bold">Round {round}: Coding Round</h1>
